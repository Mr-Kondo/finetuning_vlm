"""Vendored ``JSONParseEvaluator`` from clovaai/donut, pinned by commit SHA.

Upstream:
    repository: https://github.com/clovaai/donut
    commit:     4cfcf972560e1a0f26eb3e294c8fc88a0d336626
    file:       donut/util.py
    source URL: https://raw.githubusercontent.com/clovaai/donut/4cfcf972560e1a0f26eb3e294c8fc88a0d336626/donut/util.py
    retrieved:  2026-08-13 (sha256 of the whole upstream file:
                5b43e982264dc119e5e39a533aafba460e88d44653b62c8c5b0670cec6612e5c)

Vendored per ``docs/proposals/phase1_closure_prereg.md`` §6.2 (P-12) so that
TED-Acc and Donut micro-F1 are computed by the official implementation at a
fixed commit rather than by a lookalike reimplementation.

Deviations from upstream, all structural — the class body below is byte-for-byte
identical to upstream lines 138-340:

1. **Only ``JSONParseEvaluator`` is extracted.** ``donut/util.py`` also defines
   ``DonutDataset``, and therefore imports ``torch``, ``datasets`` and
   ``transformers`` at module level. Vendoring the file wholesale would drag the
   whole training stack into the evaluation path, so the unrelated module-level
   code is omitted.
2. **Imports are reduced to what this class actually uses** (``zss``,
   ``zss.Node``, ``nltk.edit_distance`` and three ``typing`` names). Upstream's
   ``json``/``os``/``random``/``defaultdict``/``Tuple`` imports are unused here.

No logic is modified. The wrapper that adapts this evaluator to this project's
parsing, normalization and invalid-output rules lives in
``src/vlm_lab/evaluation.py``; per §6.2 those wrapper rules — not this file —
are the documented difference from stock Donut.
"""
